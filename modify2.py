...
css_start_idx = next(i for i, l in enumerate(lines) if "/* CONNECT */" in l)
css_end_idx = next(i for i, l in enumerate(lines) if "/* FOOTER */" in l)
html_start_idx = next(i for i, l in enumerate(lines) if "<!-- CONNECT -->" in l)
html_end_idx = next(i for i, l in enumerate(lines) if "<!-- FOOTER -->" in l)

new_html = """  <!-- CONNECT & GROW -->
  <section id="connect">
...
# Perform replacement backward securely
lines[html_start_idx:html_end_idx] = [new_html]
lines[css_start_idx:css_end_idx] = [new_css]

with open(file_path, 'w', encoding='utf-8') as f:
    f.writelines(lines)

print("HTML lines:", html_start_idx, "-", html_end_idx)
print("CSS lines:", css_start_idx, "-", css_end_idx)
print("done2")
